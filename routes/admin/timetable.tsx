...
export const handler: Handlers = {
  async POST(req) {
    ...
    if (existing && existing.length > 0) {
      return new Response("Class already exists at this date and time", {
        status: 400,
      });
    }
  ...
  async PUT(req) {
    ...
    if (
      !id || !classname || !date || !start_time || !end_time || !price ||
      !max_bookings || !type || !instructor
    ) {
      return new Response("Missing fields", { status: 400 });
    }
    ...
    const { error } = await db.from("timetable").update(updateFields).eq(
      "id",
      id,
    );
      ...
      <div>
        <form method="GET" className="max-w-md mx-auto m-6">
          ...
          >
            ...
            {[
              ...new Set(data.instructors.map((ins: { instructor: string }) =>
                ins.instructor
              )),
            ].map((instructor) => (
